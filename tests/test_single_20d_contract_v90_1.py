...
class Single20DayContractV901Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_contract_has_one_objective_and_one_horizon(self):
        ...
        self.assertEqual(contract["implementation_version"], "v90.1")
    ...
    def test_v90_1_risk_and_capital_flow_diagnostics_are_present(self):
        ...
        for item in available:
            ...
            self.assertIn("capital_flow_5d_pct", forecast)
            self.assertIn("capital_flow_20d_pct", forecast)
            self.assertIn("turnover_acceleration_5v20", forecast)
            self.assertIn("capital_flow_rank", item)

        flow = self.result["model"]["market_capital_flow"]
        self.assertIn(flow["status"], ("市場資金偏流入", "市場資金偏流出", "市場資金分歧"))
        self.assertGreaterEqual(flow["positive_5d_pct"], 0)
        self.assertLessEqual(flow["positive_5d_pct"], 100)

    def test_v90_1_rank_penalises_overheating_without_unvalidated_flow_weight(self):
        self.assertEqual(len(predictor.FEATURE_NAMES), 15)
        self.assertEqual(len(predictor.FACTOR_WEIGHTS), 15)
        ...
        self.assertEqual(weights["capital_flow_5d_pct"], 0)
        self.assertEqual(weights["capital_flow_20d_pct"], 0)
        self.assertEqual(weights["turnover_acceleration_5v20"], 0)
    ...
    def test_frontend_reads_only_active_caches(self):
        ...
        self.assertIn('id="loadCacheBtn"', html)
        self.assertIn('id="rankingBtn"', html)
        self.assertIn("showLoadedSummary()", script)
        self.assertIn("capital_flow_rank", script)
        for obsolete in (
