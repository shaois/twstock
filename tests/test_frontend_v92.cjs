...
(async () => {
  ...
  assert.match(element("aiContent").textContent, /覆核未通過/);
  assert.doesNotMatch(element("aiContent").textContent, /AI建議：等待/);
  ...
  assert.doesNotMatch(element("aiContent").textContent.split('原始AI回答')[0], /投入5%|AI建議：可考慮買進/);
  assert.match(element("aiContent").textContent, /原始AI回答.*[\s\S]*投入5%/);
