...
def fetch_fundamental(stock_id):
    ...
    url = f"{BASE_URL}?dataset=TaiwanStockFinancialStatements&data_id={stock_id}&start_date={start}&token={FINMIND_TOKEN}"
    return fetch(url)

def fetch_revenue(stock_id):
    start = (datetime.today() - timedelta(days=400)).strftime("%Y-%m-%d")
    url = f"{BASE_URL}?dataset=TaiwanStockMonthRevenue&data_id={stock_id}&start_date={start}&token={FINMIND_TOKEN}"
    return fetch(url)

def fetch_price(stock_id):
    start = (datetime.today() - timedelta(days=270)).strftime("%Y-%m-%d")
    url = f"{BASE_URL}?dataset=TaiwanStockPrice&data_id={stock_id}&start_date={start}&token={FINMIND_TOKEN}"
    return fetch(url)
...
def fetch_exdiv(stock_id):
    ...
    url = f"{BASE_URL}?dataset=TaiwanStockDividend&data_id={stock_id}&start_date={start}&end_date={end}&token={FINMIND_TOKEN}"
    return fetch(url)

def build_exdiv_map(all_exdiv_data: dict) -> dict:
    """從各股的除息原始資料，整理成 { "2603": { "date": "2026-06-17", "div": 16.0}, ... }"""
    ...
    for stock_id, rows in all_exdiv_data.items():
        ...
        for row in rows:
            ...
            if not ex_date_str or cash_div <= 0: continue
            try:
...
def main():
    ...
    for i, sid in enumerate(STOCK_IDS):
        ...
        if f_data and f_data.get("_error") == 402: quota_exhausted = True; continue
        if f_data and f_data.get("status") == 200:
            result_fundamental[sid] = f_data.get("data", [])
            time.sleep(0.8)

        r_data = fetch_revenue(sid)
        if r_data and r_data.get("_error") == 402: quota_exhausted = True; continue
        if r_data and r_data.get("status") == 200:
            result_revenue[sid] = r_data.get("data", [])
            time.sleep(0.8)

        p_data = fetch_price(sid)
        if p_data and p_data.get("_error") == 402: quota_exhausted = True; continue
