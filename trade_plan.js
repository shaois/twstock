...
function buildTradePlan(item, rows, proposedPrice = null) {
  ...
  const plan = {version: "execution-scenario-1", decision: "資料不足，不買", reasons: [],
  ...
  if (!(stop>0 && ceiling>lower && target>ceiling)) {
    plan.decision="不買：目前沒有符合盈虧比的買區";
    return fail("均線買區與成本後2:1盈虧比條件無交集，不拉高目標價硬湊");
  }
  const entry = proposedPrice === null ? item.current_price : proposedPrice;
  if (!finite(entry) || entry<=0) return fail("輸入價格必須大於零");
  Object.assign(plan,{evaluated_price:entry,price_basis:proposedPrice===null?"訊號日收盤價，非即時價格":"使用者輸入的假設成交價，未查證即時報價",
    target_net_pct:(target/entry-1)*100-0.6,
    stop_net_pct:(stop/entry-1)*100-0.6});
  plan.execution_reward_risk = plan.stop_net_pct<0?plan.target_net_pct/-plan.stop_net_pct:null;
  ...
  if (entry<=stop) plan.reasons.push("假設價格已跌破情境停損，原支撐方案失效");
  const blocked = plan.reasons.length>0;
  if (blocked) plan.decision="不買：目前不符合交易條件";
  ...
  plan.exit_plan="觸及20日歷史壓力目標時評估停利；跌破支撐緩衝停損位時退出方案。目標是歷史壓力，不是預測必達價。";
  return plan;
}
