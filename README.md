...
5. V88.2 保留正式 60% 一致性門檻；若只有這一項且恰好差 1 個正報酬期間、其他開發與封存檢查全過，才開放最多 2 檔、每檔最多 5% 的條件式布局。
6. 條件式布局只能從原始前 2.5% 排名中刪除不符合風險診斷者，不能提升外部股票或改寫排名。
7. 基準日單日漲幅達 7%，或漲幅達 5% 且收盤位於當日區間頂端 95%，只保留研究排名並改為「等待回測，禁止追價」。
8. `index.html` 與 `app.js` 只讀取 `universe.json` 和 `predictions.json`。
9. AI 20 日分析只解釋模型結果，不參與排名，也不能改變模型結論。
...
- V88.2 不沿用舊版本持有名單；不符合可靠度或防追高進場門檻時允許零持股。
