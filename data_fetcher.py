...
SECTOR_PE_AVG = {"半導體": 22, "IC設計": 25, "電子製造": 15, "電腦": 18, "金融": 12, "電信": 18, "石化": 12, "鋼鐵": 10, "汽車": 15, "食品": 20, "零售": 22, "通路": 14, "紡織": 12, "橡膠": 12, "製鞋": 18, "資訊服務": 16, "光學": 35, "機殼": 14, "工業電腦": 28, "電子零組件": 20}
...
class TWStockFetcher:
    ...
    def _get_sector(self, stock_id):
        m = {"2330": "半導體", "2454": "半導體", "2303": "半導體", "3711": "半導體", "2408": "半導體", "3008": "光學", "2317": "電子製造", "2382": "電子製造", "4938": "電子製造", "2357": "電腦", "2353": "電腦", "2376": "電腦", "2327": "電子零組件", "2395": "工業電腦", "2379": "IC設計", "2308": "電子零組件", "2474": "機殼", "2385": "電子零組件", "2881": "金融", "2882": "金融", "2886": "金融", "2884": "金融", "2891": "金融", "2892": "金融", "5880": "金融", "2885": "金融", "2883": "金融", "2887": "金融", "2801": "金融", "5876": "金融", "2880": "金融", "2888": "金融", "2890": "金融", "2889": "金融", "2820": "金融", "2412": "電信", "1301": "石化", "1303": "石化", "1326": "石化", "6505": "石化", "2002": "鋼鐵", "2207": "汽車", "2201": "汽車", "2105": "橡膠", "1402": "紡織", "1216": "食品", "2912": "零售", "2347": "通路", "9910": "製鞋", "2352": "資訊服務"}
    ...
    def _read_gh_cache(self, dtype: str, stock_id: str):
        ...
        try:
            ...
            if stock_data: return {"status": 200, "data": stock_data}
    ...
    async def fetch_top50_stocks(self):
        ...
        if not stocks:
            for sid in ["2330", "2317", "2454", "2308", "2382"]:
    ...
    async def fetch_technical(self, stock_id: str) -> dict:
        ...
        macd_val, sig_val, hist = 0.0, 0.0, 0.0
        if len(closes) >= 26:
            ...
            ml = [a - b for a, b in zip(e12, e26)]; sig = calc_ema(ml, 9)
            macd_val, sig_val, hist = ml[-1], sig[-1], ml[-1] - sig[-1]
    ...
    async def fetch_fundamental(self, stock_id: str) -> dict:
        ...
        eps, roe, cash_dividend, exdiv_date = 0, 0, 0, ""
            ...
            if data:
                for row in reversed(data):
                    if row.get("type") == "EPS" and row.get("value"):
                        eps = float(row["value"]); break
                for row in reversed(data):
                    if row.get("type") == "ReturnOnEquity" and row.get("value"):
                        roe = float(row["value"]); break
                    ...
                    for row in resp.json():
                        if str(row.get("公司代號", "")).strip() == stock_id:
                            for key in ["去年同月增減(%)", "較上年同月增減%", "YoY"]:
                                if key in row:
                                    try: revenue_yoy = float(str(row[key]).replace(",", "").replace("%", "").strip())
                                    except: pass
                                    break
        except Exception: pass
